/** @wae/wae-linux-x64 — native host shell (lib/linux-x64-gnu.node). */
...
const PACKAGE_NAME = "@wae/wae-linux-x64";
const NATIVE_LIB = "linux-x64-gnu.node";
...
async function runDesktop(url: string, title: string, undecorated = false): Promise<void> {
    ...
    if (!native) {
        throw new Error(`[${PACKAGE_NAME}] lib/linux-x64-gnu.node missing. From WAE repo run: pnpm run build:native`);
    }
