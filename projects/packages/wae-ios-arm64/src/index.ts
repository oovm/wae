/** @wae/wae-ios-arm64 — native host shell (lib/ios-arm64.node). */
...
const PACKAGE_NAME = "@wae/wae-ios-arm64";
const NATIVE_LIB = "ios-arm64.node";
...
async function runDesktop(url: string, title: string, undecorated = false): Promise<void> {
    ...
    if (!native) {
        throw new Error(`[${PACKAGE_NAME}] lib/ios-arm64.node missing. From WAE repo run: pnpm run build:native`);
    }
