use semver::Version;
use wae_updater::{
    DownloadPolicy, GitHubReleaseAsset, GitHubReleaseSource, ReleaseChannel, Updater, UpdaterConfig, normalize_release_tag,
    pick_product_asset,
};
...
fn pick_asset_prefers_triple_zip() {
    let assets = vec![
        ...
        GitHubReleaseAsset { name: "other.zip".into(), browser_download_url: "https://example.com/b.zip".into(), size: 1 },
    ];
    let picked = pick_product_asset("my-app", "x86_64-pc-windows-msvc", "windows-x86_64", &assets);
    assert_eq!(picked.unwrap().name, "my-app-x86_64-pc-windows-msvc.zip");
}

#[test]
fn normalize_strips_v_prefix() {
    assert_eq!(normalize_release_tag("v1.2.3").unwrap(), Version::new(1, 2, 3));
}
...
fn channel_parse_and_policy() {
    ...
    assert_eq!(ReleaseChannel::parse("nightly-2026-09-26").unwrap(), ReleaseChannel::Pinned("nightly-2026-09-26".into()));
    assert_eq!(DownloadPolicy::parse("silent").unwrap(), DownloadPolicy::DownloadIfAvailable);
    assert_eq!(DownloadPolicy::parse("explicit").unwrap(), DownloadPolicy::CheckOnly);
}
