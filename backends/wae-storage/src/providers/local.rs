...
impl StorageProvider for LocalStorageProvider {
    fn sign_url(&self, path: &str, config: &StorageConfig) -> StorageResult<Url> {
        ...
        let base_url = Url::parse(base_url_str).map_err(|e| WaeError::invalid_params(e.to_string()))?;
        let url = base_url.join(path.trim_start_matches('/')).map_err(|e| WaeError::invalid_params(e.to_string()))?;
    ...
    fn get_presigned_put_url(&self, key: &str, config: &StorageConfig) -> StorageResult<Url> {
        ...
        let base_url = Url::parse(base_url_str).map_err(|e| WaeError::invalid_params(e.to_string()))?;
        let url = base_url.join(key.trim_start_matches('/')).map_err(|e| WaeError::invalid_params(e.to_string()))?;
