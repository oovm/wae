...
/// AWS S3 存储提供商
///
/// 实现 AWS S3 对象存储服务的签名和预签名 URL 生成
...
impl StorageProvider for S3Provider {
    fn sign_url(&self, path: &str, config: &StorageConfig) -> StorageResult<Url> {
        ...
        let signing_key = Self::get_signature_key(&config.secret_key, &date_stamp, &config.region, "s3");
        let signature = Self::hex_encode(&Self::hmac_sha256(&signing_key, string_to_sign.as_bytes()));

        url.set_query(Some(&format!("{}&X-Amz-Signature={}", canonical_querystring, signature)));
    ...
    fn get_presigned_put_url(&self, key: &str, config: &StorageConfig) -> StorageResult<Url> {
        ...
        let host_str =
            config.endpoint.clone().unwrap_or_else(|| format!("https://{}.s3.{}.amazonaws.com", config.bucket, config.region));
