...
fn bench_hash(c: &mut Criterion) {
    ...
    group.bench_function("sha1", |b| {
        b.iter(|| hash(black_box(HashAlgorithm::SHA1), black_box(data)))
    });
    
    group.bench_function("sha256", |b| {
        b.iter(|| hash(black_box(HashAlgorithm::SHA256), black_box(data)))
    });
    
    group.bench_function("sha384", |b| {
        b.iter(|| hash(black_box(HashAlgorithm::SHA384), black_box(data)))
    });
    
    group.bench_function("sha512", |b| {
        b.iter(|| hash(black_box(HashAlgorithm::SHA512), black_box(data)))
    });
...
fn bench_hmac(c: &mut Criterion) {
    ...
    group.bench_function("verify_sha256", |b| {
        b.iter(|| hmac_verify(
            black_box(HmacAlgorithm::SHA256), 
            black_box(secret), 
            black_box(data), 
            black_box(&signature)
        ))
...
fn bench_base64(c: &mut Criterion) {
    ...
    group.bench_function("decode", |b| {
        b.iter(|| base64_decode(black_box(&encoded)))
    });
    
    group.bench_function("url_encode", |b| {
        b.iter(|| base64url_encode(black_box(data)))
    });
    
    let url_encoded = base64url_encode(data);
    
    group.bench_function("url_decode", |b| {
        b.iter(|| base64url_decode(black_box(&url_encoded)))
    });
...
fn bench_password(c: &mut Criterion) {
    ...
    group.bench_function("hash_bcrypt", |b| {
        b.iter(|| hasher.hash_password(black_box(password)))
    });
    
    let hashed = hasher.hash_password(password).unwrap();
    
    group.bench_function("verify_bcrypt", |b| {
        b.iter(|| hasher.verify_password(black_box(password), black_box(&hashed)))
    });
