...
use crate::saml::{
    AuthnContextComparison, SamlAuthnRequest, SamlConfig, SamlError, SamlLogoutRequest, SamlLogoutResponse, SamlNameIdPolicy,
    SamlRequestedAuthnContext, SamlResponse, SamlResult, SpMetadataBuilder,
};
...
impl SamlService {
    ...
    pub fn process_authn_response(&self, saml_response: &str) -> SamlResult<SamlResponse> {
        let decoded = BASE64_STANDARD.decode(saml_response).map_err(|e| SamlError::Base64DecodeError(e.to_string()))?;

        let xml = String::from_utf8(decoded).map_err(|e| SamlError::XmlParsingError(e.to_string()))?;

        let response: SamlResponse = quick_xml::de::from_str(&xml).map_err(|e| SamlError::XmlParsingError(e.to_string()))?;
    ...
    fn validate_response(&self, response: &SamlResponse) -> SamlResult<()> {
        if !response.is_success() {
            return Err(SamlError::InvalidResponse(format!("SAML response status: {:?}", response.status.code())));
        }

        if self.config.validate_issuer {
            if response.issuer != self.config.idp.entity_id {
                return Err(SamlError::IssuerValidationFailed {
            ...
            if self.config.validate_audience {
                if let Some(ref restriction) = conditions.audience_restriction {
                    if !restriction.audience.contains(&self.config.sp.entity_id) {
                        return Err(SamlError::AudienceValidationFailed {
    ...
    pub fn create_logout_request_url(&self, name_id: &str, session_index: Option<&str>) -> SamlResult<String> {
        let slo_url =
            self.config.idp.slo_url.as_ref().ok_or_else(|| SamlError::ConfigurationError("SLO URL not configured".into()))?;
    ...
    pub fn process_logout_response(&self, saml_response: &str) -> SamlResult<SamlLogoutResponse> {
        ...
        let response: SamlLogoutResponse =
            quick_xml::de::from_str(&xml).map_err(|e| SamlError::XmlParsingError(e.to_string()))?;
    ...
    fn serialize_authn_request(&self, request: &SamlAuthnRequest) -> SamlResult<String> {
        quick_xml::se::to_string(request).map_err(|e| SamlError::XmlParsingError(e.to_string()))
    }
    ...
    fn encode_redirect_request(&self, xml: &str) -> SamlResult<String> {
        ...
        encoder.write_all(xml.as_bytes()).map_err(|e| SamlError::CompressionError(e.to_string()))?;
        let compressed = encoder.finish().map_err(|e| SamlError::CompressionError(e.to_string()))?;
    ...
    pub fn decode_redirect_request(&self, encoded: &str) -> SamlResult<String> {
        let decoded = BASE64_STANDARD.decode(encoded).map_err(|e| SamlError::Base64DecodeError(e.to_string()))?;

        let mut decoder = DeflateDecoder::new(&decoded[..]);
        let mut decompressed = String::new();
        decoder.read_to_string(&mut decompressed).map_err(|e| SamlError::CompressionError(e.to_string()))?;
