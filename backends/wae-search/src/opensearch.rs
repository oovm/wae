...
use opensearch::{OpenSearch, http::transport::{Transport, SingleNodeConnectionPool}, SearchParts, IndexParts, GetParts, UpdateParts, DeleteParts, BulkParts};
...
impl OpenSearchBackend {
    /// 创建新的 OpenSearch 搜索后端
    pub fn new(os_config: OpenSearchConfig, search_config: SearchConfig) -> SearchResult<Self> {
        let transport = Transport::single_node(&os_config.url)
            .map_err(|e| wae_types::WaeError::internal(format!("Failed to create OpenSearch transport: {}", e)))?;
...
impl SearchBackend for OpenSearchBackend {
    ...
    async fn create_index(&self, index: &str, settings: Option<Value>, mappings: Option<Value>) -> SearchResult<bool> {
        ...
        let response = self.client.indices().create(opensearch::indices::IndicesCreateParts::Index(index))
            .body(body)
            .send()
            .await
    ...
    async fn delete_index(&self, index: &str) -> SearchResult<bool> {
        let response = self.client.indices().delete(opensearch::indices::IndicesDeleteParts::Index(index))
            .send()
            .await
    ...
    async fn index_exists(&self, index: &str) -> SearchResult<bool> {
        let response = self.client.indices().exists(opensearch::indices::IndicesExistsParts::Index(index))
            .send()
            .await
    ...
    async fn get_index(&self, index: &str) -> SearchResult<Value> {
        let response = self.client.indices().get(opensearch::indices::IndicesGetParts::Index(index))
            .send()
            .await
    ...
    async fn delete_by_query(&self, index: &str, query: Value) -> SearchResult<u64> {
        let response = self.client.delete_by_query(opensearch::DeleteByQueryParts::Index(&[index]))
    ...
    async fn update_by_query(&self, index: &str, query: Value, script: Value) -> SearchResult<u64> {
        ...
        let response = self.client.update_by_query(opensearch::UpdateByQueryParts::Index(&[index]))
            .body(body)
            .send()
            .await
...
/// 便捷函数：创建 OpenSearch 搜索服务
pub fn opensearch_search(os_config: OpenSearchConfig, search_config: SearchConfig) -> SearchResult<crate::SearchService> {
    let backend = OpenSearchBackend::new(os_config, search_config)?;
