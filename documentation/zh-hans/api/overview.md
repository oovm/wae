...
```rust
// 使用 all-in-one
use wae::{ai, storage, email, https, CloudError, CloudResult};
...
| 模块 | 说明 |
|-----|------|
| `wae-types` | 核心类型定义 (CloudError, CloudResult, BillingDimensions) |
...
| Enum | XxxType / XxxError / XxxState | StorageProviderType, CloudError, CircuitState |
| Result | XxxResult | AiResult, StorageResult, AuthResult |
| 错误 | XxxError | AuthError, WebSocketError, ConfigError |
...
async fn handle_request() -> Result<(), CloudError> {
